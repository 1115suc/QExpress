...
import course.QExpress.transport.domain.TransportLineSearchDTO;
import course.QExpress.transport.entity.line.TransportLine;
...
import course.QExpress.transport.enums.DispatchMethodEnum;
import course.QExpress.transport.enums.ExceptionEnum;
import course.QExpress.transport.enums.TransportLineEnum;
import course.QExpress.transport.repository.TransportLineRepository;
...
import course.QExpress.transport.service.TransportLineService;
...
public class TransportLineServiceImpl implements TransportLineService {
    ...
    public Boolean createLine(TransportLine transportLine) {
        ...
        if (ObjectUtil.equal(transportLine.getStartOrganId(), transportLine.getEndOrganId())) {
            //起点终点不能相同
            throw new QEException(ExceptionEnum.TRANSPORT_LINE_ORGAN_CANNOT_SAME);
        }
        ...
        if (ObjectUtil.hasEmpty(firstNode, secondNode)) {
            throw new QEException(ExceptionEnum.START_END_ORGAN_NOT_FOUND);
        }

        //判断路线是否已经存在
        Long count = this.transportLineRepository.queryCount(firstNode, secondNode);
        if (count > 0) {
            throw new QEException(ExceptionEnum.TRANSPORT_LINE_ALREADY_EXISTS);
        }
    ...
    private void infoFromMap(BaseEntity firstNode, BaseEntity secondNode, TransportLine transportLine) {
        ...
        if (ObjectUtil.hasEmpty(startOrgan, startOrgan.getLongitude(), startOrgan.getLatitude())) {
            throw new QEException("请先完善机构信息");
        }
    ...
    public Boolean updateLine(TransportLine transportLine) {
        ...
        if (null == transportLineData) {
            throw new QEException(ExceptionEnum.TRANSPORT_LINE_NOT_FOUND);
        }
