...
public class AuthServiceImpl implements AuthService {
    ...
    public void create(String key, HttpServletResponse response) throws IOException {
        if (StringUtils.isBlank(key)) {
            throw new QEWebException("验证码key不能为空");
        }
    ...
    public boolean check(String key, String value) {
        if (StringUtils.isBlank(value)) {
            throw new QEWebException("请输入验证码");
        }
        String code = redisTemplate.opsForValue().get(key);
        if (code == null) {
            throw new QEWebException("验证码已过期");
        }
        if (!StringUtils.equalsIgnoreCase(value, code)) {
            throw new QEWebException("验证码不正确");
        }
    ...
    public PageResponse<SysUserVO> findCourierByPage(Integer page,
        ...
        Map<Long, Boolean> workMap = workSchedulingDTOS.parallelStream()
                .filter(workSchedulingDTO -> workSchedulingDTO.getWorkSchedules() != null)
    ...
    private PageResponse<SysUserVO> getPageResponseR(@RequestParam(name = "page") Integer page, @RequestParam(name = "pageSize") Integer pageSize, Result<PageDTO<UserDTO>> result) {
        if (result.getCode() == 0 && result.getData() != null) {
            ...
            return PageResponse.<SysUserVO>builder().items(voList).page(page).pageSize(pageSize).counts(userPage.getTotal()).pages(userPage.getTotal() % userPage.getSize()).build();
        }
