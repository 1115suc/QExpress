package course.QExpress.web.manager.controller.base.truck;

import course.QExpress.common.vo.R;
import course.QExpress.web.manager.service.BaseTruckService;
import course.QExpress.web.manager.vo.baseTruck.BindingTrucksVO;
import course.QExpress.web.manager.vo.baseTruck.TransportTripsUpdateVO;
...
public class TruckTripsController {
    ...
    public R<List<TransportTripsVO>> findAllTransportLineTrips(@RequestParam(name = "transportLineId", required = false) Long transportLineId) {
        List<TransportTripsVO> allTransportLineTrips = baseTruckService.findAllTrips(transportLineId);
        return R.success(allTransportLineTrips);
    }
    ...
    public R<TransportTripsVO> findTransportLineTripsById(@PathVariable(name = "id") Long id) {
        TransportTripsVO transportLineTripsById = baseTruckService.findTripsDetail(id);
        return R.success(transportLineTripsById);
    }
    ...
    public R deleteTransportLineTrips(@PathVariable(name = "id") Long id) {
    ...
    public R saveTransportTripsTruck(@PathVariable(name = "id") Long id, @RequestBody BindingTrucksVO bindingTrucksVO) {
        //保存车辆安排信息
    ...
    public R<List<TruckVO>> getTransportTripsTruck(Long transportTripsId) {
        List<TruckVO> truckVOS = baseTruckService.getTransportTripsTruck(transportTripsId);
