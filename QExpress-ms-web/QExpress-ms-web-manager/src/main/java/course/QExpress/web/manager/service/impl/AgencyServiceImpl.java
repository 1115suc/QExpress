...
import course.QExpress.scope.api.ServiceScopeFeign;
import course.QExpress.scope.domain.dto.ServiceScopeDTO;
...
import course.QExpress.web.manager.vo.baseCommon.AreaVO;
...
public class AgencyServiceImpl implements AgencyService {
    ...
    public OrganDTO parseAgencyVOToDTO(AgencyUpdateVO agencyVO) {
        ...
        boolean provinceOk = agencyVO.getProvinceId() != null && agencyVO.getProvinceId() != 0;
        boolean cityOk = agencyVO.getCityId() != null && agencyVO.getCityId() != 0;
        boolean countyOk = agencyVO.getCountyId() != null && agencyVO.getCountyId() != 0;
        if (provinceOk) {
    ...
    public void deleteScopeById(Long id, Integer type) {
        try {
            ...
            throw new QEWebException("删除失败，不存在的围栏");
        }
    }
    ...
    public void saveScope(ServiceScopeVO vo) {
        ...
        try {
            ...
            throw new QEWebException("保存失败, 请正确绘制没有交叉点的规则多边形");
        }
    ...
    public ServiceScopeVO findScope(Long id, Integer type) {
        ...
        if (type == 2) {
            ...
            if (ObjectUtil.isEmpty(result.getData())) {
                throw new QEWebException("id为" + id + "的快递员不存在！");
            }
