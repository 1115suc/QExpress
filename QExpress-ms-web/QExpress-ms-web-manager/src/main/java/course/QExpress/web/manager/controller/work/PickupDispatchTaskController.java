...
import course.QExpress.web.manager.vo.work.PickupDispatchTaskListVO;
import course.QExpress.web.manager.vo.work.PickupDispatchTaskQuantityStatisticsVO;
import course.QExpress.web.manager.vo.work.TaskPickupDispatchQueryVO;
...
public class PickupDispatchTaskController {
    ...
    @ApiOperation(value = "取派件任务数量统计")
    @GetMapping("/quantityStatistics/{taskType}")
    @ApiImplicitParams({@ApiImplicitParam(name = "taskType", value = "任务类型，1取件，2派件", required = true, dataTypeClass = Integer.class)})
    public R<PickupDispatchTaskQuantityStatisticsVO> quantityStatistics(@PathVariable Integer taskType) {
        return R.success(workService.quantityStatistics(taskType));
    }
