...
import course.QExpress.ms.base.api.truck.TruckPlanFeign;
import course.QExpress.ms.base.api.user.DriverFeign;
import course.QExpress.ms.base.domain.enums.StatusEnum;
...
import course.QExpress.web.manager.service.*;
import course.QExpress.web.manager.vo.agency.AgencySimpleVO;
import course.QExpress.web.manager.vo.agency.AgencyVO;
import course.QExpress.web.manager.vo.auth.SysUserVO;
import course.QExpress.web.manager.vo.baseTruck.TransportTripsVO;
import course.QExpress.web.manager.vo.baseTruck.TruckVO;
import course.QExpress.web.manager.vo.oms.OrderVO;
import course.QExpress.web.manager.vo.transportLine.TransportLineVO;
import course.QExpress.web.manager.vo.work.*;
import course.QExpress.work.api.PickupDispatchTaskFeign;
...
import course.QExpress.work.domain.dto.TaskTransportUpdateDTO;
import course.QExpress.work.domain.dto.TransportOrderDTO;
import course.QExpress.work.domain.dto.TransportTaskDTO;
...
import course.QExpress.work.domain.dto.request.TransportTaskPageQueryDTO;
import course.QExpress.work.domain.dto.response.TransportOrderStatusCountDTO;
import course.QExpress.work.domain.dto.response.TransportTaskStatusCountDTO;
import course.QExpress.work.domain.enums.transporttask.TransportTaskLoadingStatus;
...
public class WorkServiceImpl implements WorkService {
    ...
    public PageResponse<TransportOrderVO> findTransportOrderByPage(TransportOrderQueryVO vo) {
        ...
        List<TransportOrderVO> voList = batchParseTransportOrderDTO2Vo(dtoList);
    ...
    private List<TaskTransportVO> batchParseTransportTaskDTO2Vo(List<TransportTaskDTO> dtoList) {
        ...
        if (CollUtil.isNotEmpty(lineIds)) {
            transportLineVOMap = transportLineService.findTransportLineByIds(lineIds).parallelStream().collect(Collectors.toMap(TransportLineVO::getId, v -> v));
        }
    ...
    public PageResponse<TaskTransportVO> findTaskByPage(TaskTransportQueryVO vo) {
        ...
        List<TransportTaskDTO> dtoList = dtoPageResponse.getItems();
        List<TaskTransportVO> taskTransportVOS = batchParseTransportTaskDTO2Vo(dtoList);
        return PageResponse.<TaskTransportVO>builder().items(taskTransportVOS).pageSize(vo.getPageSize()).page(vo.getPage()).counts(dtoPageResponse.getCounts()).pages(dtoPageResponse.getPages()).build();
    }
    ...
    public PageResponse<PickupDispatchTaskListVO> findTaskPickupDispatchByPage(TaskPickupDispatchQueryVO vo) {
        ...
        //3.dto转为vo
        List<PickupDispatchTaskListVO> list = pageResponse.getItems().stream().map(dto -> {
            PickupDispatchTaskListVO responseVO = BeanUtil.toBean(dto, PickupDispatchTaskListVO.class);
            //机构信息
        ...
        //4.组装分页结果
        return PageResponse.<PickupDispatchTaskListVO>builder()
                .items(list)
                .page(vo.getPage())
     ...
     * 取派件任务数量统计
     *
     * @param taskType 任务类型，1取件，2派件
     * @return 数量统计
     */
    @Override
    public PickupDispatchTaskQuantityStatisticsVO quantityStatistics(Integer taskType) {
        return null;
    }
    ...
    public TaskPickupDispatchVO parsePickupDispatchTaskDTO2Vo(PickupDispatchTaskDTO dto) {
        TaskPickupDispatchVO vo = BeanUtil.copyProperties(dto, TaskPickupDispatchVO.class);
        vo.setStatus(dto.getStatus().getCode());
        vo.setAssignedStatus(dto.getAssignedStatus().getCode());
    ...
    public void updateTaskPickupDispatchById(List<String> ids, Long courierId) {
        ids.forEach(id -> pickupDispatchTaskFeign.updateCourierId(Long.valueOf(id), 0L, courierId));
    }
    ...
    public void cancel(Long id) {
        ...
        if (ObjectUtil.notEqual(TransportTaskLoadingStatus.EMPTY, transportTaskDTO.getLoadingStatus())) {
            throw new QEWebException("运输任务不是空载，无法取消！");
        }

        //1.2非待执行状态无法取消
        if (ObjectUtil.notEqual(TransportTaskStatus.PENDING, transportTaskDTO.getStatus())) {
            throw new QEWebException("运输任务不是待执行状态，无法取消！");
        }
    ...
    public void saveConfiguration(DispatchConfigurationVO vo) {
        ...
        stringRedisTemplate.opsForValue().set(dispatchTimeRedisKey, String.valueOf(vo.getDispatchTime()));

        //调度方式配置
        String dispatchMethodRedisKey = "DISPATCH_CONFIGURATION:METHOD";
        stringRedisTemplate.opsForValue().set(dispatchMethodRedisKey, String.valueOf(vo.getDispatchMethod()));
    }
    ...
    public DispatchConfigurationVO findConfiguration() {
        ...
        //组装响应结果
        return DispatchConfigurationVO.builder()
                //如果查不到调度时间，默认值为2小时
