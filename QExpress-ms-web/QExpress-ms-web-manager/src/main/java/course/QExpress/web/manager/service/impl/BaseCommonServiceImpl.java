...
import course.QExpress.ms.base.api.common.GoodsTypeFeign;
import course.QExpress.ms.base.api.common.WorkPatternFeign;
import course.QExpress.ms.base.api.common.WorkSchedulingFeign;
import course.QExpress.ms.base.api.truck.TruckTypeFeign;
import course.QExpress.ms.base.domain.base.*;
import course.QExpress.ms.base.domain.truck.TruckTypeDto;
import course.QExpress.web.manager.enums.TruckTypeAllowableLoadEnum;
import course.QExpress.web.manager.enums.TruckTypeAllowableVolumeEnum;
import course.QExpress.web.manager.service.AuthService;
import course.QExpress.web.manager.service.BaseCommonService;
import course.QExpress.web.manager.util.ExcelUtil;
import course.QExpress.web.manager.vo.auth.SysUserVO;
import course.QExpress.web.manager.vo.baseCommon.*;
import course.QExpress.web.manager.vo.baseTruck.TruckTypeVO;
...
public class BaseCommonServiceImpl implements BaseCommonService {
    ...
    public Map<Long, TruckTypeVO> truckTypeMap(Set<Long> truckTypeSet) {
        ...
        return truckTypeDtoList.stream().map(truckTypeDto -> {
            ...
            BeanUtil.copyProperties(truckTypeDto, vo);
            return vo;
    ...
    public List<GoodsTypeVO> findGoodsTypeAll(List<Long> ids) {
        ...
        return goodsTypeDtoList.stream().map(goodsTypeDto -> {
            ...
            BeanUtil.copyProperties(goodsTypeDto, simpleVo);
            return simpleVo;
        }).collect(Collectors.toList());
    }
     ...
     * @param truckTypeName 车型名称
     * @return 分页货物类型数据
     */
    @Override
    public PageResponse<GoodsTypeVO> findGoodsTypeByPage(Integer page, Integer pageSize, String name, Long truckTypeId, String truckTypeName) {
        ...
        List<GoodsTypeVO> goodsTypeVOList = parseGoodsTypeDto2Vo(goodsTypeDtoList);
    ...
    public TruckTypeVO parseTruckTypeDto2Vo(TruckTypeDto dto) {
        ...
        if (ObjectUtil.isNotEmpty(dto.getGoodsTypeIds())) {
            ...
            vo.setGoodsTypes(goodsTypeDtoList.stream().map(goodsTypeDto -> {
                GoodsTypeVO goodsTypeVo = new GoodsTypeVO();
                BeanUtil.copyProperties(goodsTypeDto, goodsTypeVo);
                return goodsTypeVo;
            }).collect(Collectors.toList()));
        }
        return vo;
    ...
    public PageResponse<TruckTypeVO> findTruckTypeByPage(Integer page,
                                                         ...
                                                         Long id) {
        //构造查询条件
        BigDecimal minAllowableLoad = allowableLoad != null ? allowableLoad.getValue().getMinLoad() : null;
        BigDecimal maxAllowableLoad = allowableLoad != null ? allowableLoad.getValue().getMaxLoad() : null;
        BigDecimal minAllowableVolume = allowableVolume != null ? allowableVolume.getValue().getMinLoad() : null;
        BigDecimal maxAllowableVolume = allowableVolume != null ? allowableVolume.getValue().getMaxLoad() : null;
        ...
        List<TruckTypeVO> truckTypeVOList = parseTruckTypeDto2Vo(truckTypeDtoList);
