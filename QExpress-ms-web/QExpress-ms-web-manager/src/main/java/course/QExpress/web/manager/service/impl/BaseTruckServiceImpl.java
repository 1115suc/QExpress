package course.QExpress.web.manager.service.impl;
...
import course.QExpress.common.util.AuthTemplateThreadLocal;
import course.QExpress.common.util.Constant;
import course.QExpress.common.util.PageResponse;
import course.QExpress.ms.base.api.common.WorkSchedulingFeign;
import course.QExpress.ms.base.api.trips.TripsFeign;
import course.QExpress.ms.base.api.truck.TruckFeign;
import course.QExpress.ms.base.api.truck.TruckLicenseFeign;
import course.QExpress.ms.base.api.user.DriverFeign;
import course.QExpress.ms.base.domain.base.WorkSchedulingDTO;
import course.QExpress.ms.base.domain.enums.TruckWorkStatusEnum;
import course.QExpress.ms.base.domain.enums.WorkUserTypeEnum;
import course.QExpress.ms.base.domain.truck.TransportTripsTruckDriverDto;
import course.QExpress.ms.base.domain.truck.TruckDto;
import course.QExpress.ms.base.domain.truck.TruckLicenseDto;
import course.QExpress.ms.base.domain.truck.TruckTripsDto;
import course.QExpress.ms.base.domain.user.TruckDriverDto;
import course.QExpress.ms.base.domain.user.TruckDriverLicenseDto;
import course.QExpress.web.manager.service.AuthService;
import course.QExpress.web.manager.service.BaseCommonService;
import course.QExpress.web.manager.service.BaseTruckService;
import course.QExpress.web.manager.service.TransportLineService;
import course.QExpress.web.manager.vo.auth.SysUserVO;
import course.QExpress.web.manager.vo.baseTruck.*;
import course.QExpress.web.manager.vo.transportLine.TransportLineVO;
import course.QExpress.work.api.TransportTaskFeign;
...
public class BaseTruckServiceImpl implements BaseTruckService {
    ...
    public PageResponse<DriverVO> findDriverByPage(Integer page,
        ...
        if (CollUtil.isNotEmpty(workSchedulingDTOS)) {
            workMap = workSchedulingDTOS.parallelStream().filter(workSchedulingDTO -> workSchedulingDTO.getWorkSchedules() != null).collect(Collectors.toMap(WorkSchedulingDTO::getUserId, workSchedulingDTO -> workSchedulingDTO.getWorkSchedules().get(0)));
        }
        // 补充数据
    ...
    public void bindingTrucks(BindingDriversVO bindingDriversVO) {
        ...
        if (CollUtil.isEmpty(bindingDriversVO.getDriverIds())) {
            throw new QEWebException("配置失败，司机列表为空");
        }
    ...
    private void checkBingDriverVO(BindingDriversVO bindingDriversVO) {
        ...
        if (ObjectUtil.isEmpty(truckSimple) || ObjectUtil.isEmpty(truckSimple.getTruckLicenseId())) {
            throw new QEWebException("配置失败，请先完善车辆信息");
        }
        ...
        if (ObjectUtil.isEmpty(truckLicenseDto) || ObjectUtil.isEmpty(truckLicenseDto.getPicture())) {
            throw new QEWebException("配置失败，请上传行驶证照片");
        }
        ...
        if (CollUtil.isNotEmpty(bindingDriversVO.getDriverIds())) {
            ...
            bindingDriversVO.getDriverIds().forEach(driverId -> {
                ...
                if (ObjectUtil.isEmpty(oneDriverLicense) || ObjectUtil.isEmpty(oneDriverLicense.getPicture())) {
                    throw new QEWebException("配置失败，请上传驾驶证照片");
                }
            });
    ...
    public PageResponse<TruckVO> findTruckByPage(Integer page,
        ...
        List<TruckVO> truckVOS = batchParseTruckDto2Vo(truckDtoList);
        return PageResponse.<TruckVO>builder().items(truckVOS).page(page).pageSize(pageSize).counts(truckDtoPage.getCounts()).pages(truckDtoPage.getPages()).build();
    }
    ...
    private TruckVO parseTruckDto2Vo(TruckDto dto) {
        ...
        List<TransportLineVO> transportLineByIds = transportLineService.findTransportLineByIds(truckTripsDtos.stream().map(TruckTripsDto::getTransportLineId).distinct().collect(Collectors.toList()));
    ...
    public void disableTruck(Long id) {
        ...
        if (count > 0) {
            throw new QEWebException("存在未完成运输任务 不能解除");
        }
    ...
    public void delTruck(Long id) {
        ...
        if (CollUtil.isNotEmpty(driverVOS)) {
            throw new QEWebException("请先解除车辆下的司机");
        }
        truckFeign.del(id);
    }
    ...
    public List<TruckDriverTripsVO> findTruckTrips(Long truckId) {
        ...
        tripsFeign.findAllTruckDriverTransportTrips(null, truckId, null).forEach(transportTripsTruckDriverDto -> {
            ...
            if (ObjectUtil.isNotEmpty(transportLineTripsById)) {
                ...
                TransportLineVO lineById = transportLineService.findTransportLineById(transportLineTripsById.getTransportLine().getId());
    ...
    private TransportTripsVO parseTransportTripsDto2Vo(TruckTripsDto dto) {
        ...
        if (ObjectUtil.isNotEmpty(dto.getTransportLineId())) {
            TransportLineVO lineById = transportLineService.findTransportLineById(dto.getTransportLineId());
            vo.setTransportLine(lineById);
    ...
    public void saveTrips(TransportTripsUpdateVO vo) {
        TruckTripsDto dto = new TruckTripsDto();
     ...
     */
    @Override
    public List<TruckVO> workingTrucks() {
        ...
        List<TruckDto> truckDtos = truckFeign.findAll(null).stream().filter(v -> ObjectUtil.isNotEmpty(v.getWorkStatus()) && ObjectUtil.equal(v.getWorkStatus(), TruckWorkStatusEnum.WORKING.getCode()) && !truckSet.contains(v.getId()))
                .collect(Collectors.toList());
    ...
    public void bindingDrivers(BindingDriversVO bindingDriversVO) {
        ...
        // 车辆为空
        if (ObjectUtil.isEmpty(bindingDriversVO.getTruckId())) {
            throw new QEWebException("配置失败，车辆为空");
        }
