...
import course.QExpress.common.util.*;
...
import course.QExpress.search.api.CourierTaskFeign;
...
import course.QExpress.user.domain.enums.MemberIdCardVerifyStatus;
import course.QExpress.web.courier.constants.CourierConstants;
import course.QExpress.web.courier.enums.SortOrderEnum;
import course.QExpress.web.courier.enums.TaskInformationEnum;
import course.QExpress.web.courier.enums.TaskStatus;
import course.QExpress.web.courier.service.TaskService;
import course.QExpress.web.courier.vo.task.*;
...
public class TaskServiceImpl implements TaskService {
    ...
    public CarriageVO calculate(CarriageCalculateVO calculateVO) {
        ...
        if (CollUtil.isEmpty(areaDtos)) {
            throw new QEWebException("地址未找到");
        }
    ...
    private Map<String, String> getNameAndPhoneAndAddress(TaskInformationEnum taskInformationEnum, OrderDTO orderDTO) {
        ...
        switch (taskInformationEnum) {
            ...
            case RECEIVE://2任务关联收件人信息
                ...
                break;
            default:
                throw new QEWebException("Unexpected taskType:" + taskInformationEnum);
        }
    ...
    private void setTskTypeAndStatus(TaskQueryVO vo) {
        switch (TaskStatus.lookup(vo.getTaskStatus())) {
            ...
            case COMPLETE_DISPATCH://5已签收
                ...
                break;
            default:
                throw new QEWebException("Unexpected taskStatus:" + vo);
        }
    }
        ...
        if (ObjectUtil.isEmpty(status)) {
            ...
            switch (PickupDispatchTaskStatus.codeOf(status)) {
                ...
                case CANCELLED:
                    ...
                    break;
                default:
                    throw new QEWebException("Unexpected taskStatus:" + status);
            }
        }
    }
    ...
    public RealNameVerifyVO idCardCheck(TaskIdCardCheckVO vo) {
        ...
        if (Boolean.parseBoolean(realNameVerify)) {
            ...
            try {
                ...
                throw new QEWebException("实名认证方法执行失败");
            }
        }
    ...
    public boolean pickup(TaskPickupVO vo) {
        ...
        if (ObjectUtil.notEqual(taskDTO.getStatus(), PickupDispatchTaskStatus.NEW)) {
            throw new QEWebException("快递必须是未取状态！");
        }
        ...
        if (!memberDto.getIdCardNoVerify().equals(MemberIdCardVerifyStatus.SUCCESS.getCode())) {
            if (CharSequenceUtil.hasBlank(vo.getIdCard(), vo.getName())) {
                throw new QEWebException("用户未实名认证，必须输入身份证号和姓名！");
            }
        ...
        //7.发送取件成功的消息
        String msg = CourierMsg.builder()
                .orderId(orderId)
                .courierId(userDTO.getId())
                .courierName(userDTO.getName())
                .courierMobile(userDTO.getMobile())
                .created(System.currentTimeMillis())
                //更新订单消息放到扩展信息中，需要的业务再处理，不需要的忽略即可
                .info(orderPickupJson)
                .build().toJson();
    ...
    public void sign(TaskSignVO vo) {
        ...
        if (ObjectUtil.notEqual(taskDTO.getSignStatus(), PickupDispatchTaskSignStatus.NOT_SIGNED)) {
            throw new QEWebException("快递已签收/拒收");
        }
    ...
    public void reject(String id) {
        ...
        if (ObjectUtil.equal(transportOrderDTO.getIsRejection(), true)) {
            throw new QEWebException("快递已被收件人拒收，不能再次拒收！");
        }
