...
import course.QExpress.ms.base.domain.base.LatestMessageDTO;
import course.QExpress.ms.base.domain.base.MessageDTO;
import course.QExpress.ms.base.domain.base.MessageQueryDTO;
import course.QExpress.ms.base.domain.constants.MessageConstants;
...
import course.QExpress.web.courier.service.MessageService;
import course.QExpress.web.courier.vo.message.MessageQueryVO;
import course.QExpress.web.courier.vo.message.MessageVO;
import course.QExpress.web.courier.vo.message.MessagesHomeVO;
import course.QExpress.web.courier.vo.message.NewNoticeInfoVO;
...
public class MessageServiceImpl implements MessageService {
    ...
    public MessagesHomeVO homeMessage() {
        ...
        Integer noReadNum = noReadRes != 0 ? noReadRes : null;
        messagesHomeVO.setUnRead(noReadNum != null);
        messagesHomeVO.setNewsNum(noReadNum != null ? noReadNum : 0);

        if (latestMessageDTO.getCreated() != null) {
            messagesHomeVO.setMinutes((int) Duration.between(latestMessageDTO.getCreated(), LocalDateTime.now()).toMinutes());
        }
        if (latestMessageDTO.getContentType() != null) {
            messagesHomeVO.setContentType(latestMessageDTO.getContentType());
            switch (MessageContentTypeEnum.codeOf(latestMessageDTO.getContentType())) {
                //公告消息
                ...
                case COURIER_DISPATCH:
                    ...
                    break;
                default:
                    throw new QEWebException("Unexpected value: " + latestMessageDTO.getContentType());
            }
    ...
    public NewNoticeInfoVO notice() {
        ...
        //组装最新消息时间
        newNoticeInfoVO.setNewSendNoticeTime(pickupDTO != null ? pickupDTO.getCreated() : null);
        newNoticeInfoVO.setNewReceiveNoticeTime(signDTO != null ? signDTO.getCreated() : null);
        newNoticeInfoVO.setNewCancelNoticeTime(cancelDTO != null ? cancelDTO.getCreated() : null);
        newNoticeInfoVO.setNewDispatchNoticeTime(dispatchDTO != null ? dispatchDTO.getCreated() : null);
    ...
    public PageResponse<MessageVO> page(MessageQueryVO messageQueryVO) {
        ...
        //3.组装分页数据（添加取派件任务状态）
        List<MessageVO> list = pageResponse.getItems().stream().map(dto -> {
            MessageVO messageVO = BeanUtil.toBean(dto, MessageVO.class);

            PickupDispatchTaskDTO pickupDispatchTaskDTO = pickupDispatchTaskFeign.findById(messageVO.getRelevantId());
            messageVO.setStatus(pickupDispatchTaskDTO.getStatus());
            return messageVO;
        }).collect(Collectors.toList());

        //4.组装分页结果
        return PageResponse.<MessageVO>builder()
                .page(messageQueryVO.getPage())
                .pageSize(messageQueryVO.getPageSize())
