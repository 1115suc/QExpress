...
import course.QExpress.common.util.Result;
...
import course.QExpress.oms.domain.enums.OrderType;
...
import course.QExpress.oms.service.OrderService;
import course.QExpress.transport.api.TransportLineFeign;
import course.QExpress.user.api.AddressBookFeign;
import course.QExpress.user.domain.dto.AddressBookDTO;
...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, OrderEntity> implements OrderService {
    ...
    private HashMap<Long, AddressBookDTO> getOrderAddress(Long sendAddressId, Long receiptAddressId) {
        ...
        if (ObjectUtil.isEmpty(sendAddress) || ObjectUtil.isEmpty(receiptAddress)) {
            ...
            throw new QEException("获取地址详细信息失败");
        }
    ...
    private CarriageDTO computeCarriage(MailingSaveDTO mailingSaveDTO, Long senderCityId, Long receiverCityId) {
        ...
        if (ObjectUtil.isEmpty(compute)) {
            throw new QEException(StrUtil.format("计算运费出错 mailingSaveDTO {}", mailingSaveDTO));
        }
        return compute;
    }
     ...
     * @return
     */
    private Result getAgencyId(String address) throws QEException {

        if (ObjectUtil.isEmpty(address)) {
            log.error("地址不能为空");
            throw new QEException("下单时发货地址不能为空");
        }
        //根据详细地址查询坐标
        ...
        if (CollectionUtils.isEmpty(serviceScopeDTOS)) {
            log.error("地址不在服务范围");
            throw new QEException("地址不在服务范围");
        }
