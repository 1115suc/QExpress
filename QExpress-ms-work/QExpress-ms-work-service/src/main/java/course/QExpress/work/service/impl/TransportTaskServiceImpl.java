...
import course.QExpress.work.entity.TransportTaskEntity;
import course.QExpress.work.mapper.TransportTaskMapper;
import course.QExpress.work.service.TransportOrderTaskService;
...
import static course.QExpress.work.domain.enums.WorkExceptionEnum.TRANSPORT_TASK_CANCEL_FAILED;
...
public class TransportTaskServiceImpl extends
    ...
    public Boolean updateStatus(Long id, TransportTaskStatus status) {
        if (TransportTaskStatus.PENDING == status) {
            //修改运输任务状态不能为 待执行 状态
            throw new QEException(WorkExceptionEnum.TRANSPORT_TASK_STATUS_NOT_PENDING);
        }
        ...
        if (TransportTaskStatus.PROCESSING == status) {
            ...
            if (CollUtil.isNotEmpty(transportOrderIdList)) {
                throw new QEException(TRANSPORT_TASK_CANCEL_FAILED);
            }
    ...
    public List<TransportTaskDTO> findAllByOrderIdOrTaskId(String transportOrderId, Long transportTaskId) {
        if (ObjectUtil.isAllEmpty(transportOrderId, transportTaskId)) {
            throw new QEException(WorkExceptionEnum.TRANSPORT_TASK_QUERY_PARAM_ERROR);
        }
        ...
        if (ObjectUtil.isNotEmpty(transportTaskId)) {
            ...
            if (ObjectUtil.isEmpty(transportTask)) {
                throw new QEException(WorkExceptionEnum.TRANSPORT_TASK_NOT_FOUND);
            }
    ...
    public void adjust(TaskTransportUpdateDTO dto) {
        if (ObjectUtil.hasEmpty(dto, dto.getId())
            ...
            throw new QEException(WorkExceptionEnum.TRANSPORT_TASK_UPDATE_PARAM_ERROR);
        }
