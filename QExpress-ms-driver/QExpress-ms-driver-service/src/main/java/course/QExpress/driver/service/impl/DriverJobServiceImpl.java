package course.QExpress.driver.service.impl;
...
import course.QExpress.driver.domain.dto.request.*;
import course.QExpress.driver.domain.dto.response.DailyMileageDTO;
import course.QExpress.driver.domain.dto.response.DriverJobDTO;
import course.QExpress.driver.domain.dto.response.DriverJobStatisticsDTO;
import course.QExpress.driver.domain.enums.DriverExceptionEnum;
import course.QExpress.driver.domain.enums.DriverJobStatus;
import course.QExpress.driver.entity.DriverJobEntity;
import course.QExpress.driver.mapper.DriverJobMapper;
import course.QExpress.driver.service.DriverJobService;
import course.QExpress.ms.base.api.common.MessageFeign;
import course.QExpress.ms.base.api.truck.TruckFeign;
import course.QExpress.ms.base.api.truck.TruckPlanFeign;
import course.QExpress.ms.base.api.truck.TruckReturnRegisterFeign;
import course.QExpress.ms.base.domain.base.MessageAddDTO;
import course.QExpress.ms.base.domain.enums.MessageBussinessTypeEnum;
import course.QExpress.ms.base.domain.enums.MessageContentTypeEnum;
import course.QExpress.ms.base.domain.enums.StatusEnum;
import course.QExpress.ms.base.domain.enums.TruckRunStatusEnum;
import course.QExpress.ms.base.domain.truck.TruckDto;
import course.QExpress.ms.base.domain.truck.TruckPlanDto;
...
import course.QExpress.work.domain.dto.request.TransportTaskCompleteDTO;
import course.QExpress.work.domain.dto.request.TransportTaskDelayDeliveryDTO;
import course.QExpress.work.domain.dto.request.TransportTaskStartDTO;
import course.QExpress.work.domain.dto.response.TransportTaskMonthlyDistanceDTO;
...
public class DriverJobServiceImpl extends ServiceImpl<DriverJobMapper, DriverJobEntity> implements DriverJobService {
    ...
    public Long createDriverJob(Long transportTaskId, Long driverId) {
        ...
        if (ObjectUtil.isEmpty(transportTaskDTO)) {
            throw new QEException(DriverExceptionEnum.TRANSPORT_TASK_NOT_FOUND);
        }
        ...
        throw new QEException(DriverExceptionEnum.DRIVER_JOB_SAVE_ERROR);
    }
    ...
    public boolean updateStatus(Long id, DriverJobStatus status) {
        ...
        switch (status) {
            case PENDING: {
                throw new QEException(DriverExceptionEnum.DRIVER_JOB_STATUS_NOT_PENDING);
            }
            case PROCESSING: {
                //司机出库
    ...
    public void intoStorage(DriverDeliverDTO driverDeliverDTO) {
        ...
        if (ObjectUtil.isEmpty(driverJob)) {
            throw new QEException(DriverExceptionEnum.DRIVER_JOB_NOT_FOUND);
        }
        if (ObjectUtil.notEqual(driverJob.getStatus(), DriverJobStatus.PROCESSING)) {
            throw new QEException(DriverExceptionEnum.DRIVER_JOB_STATUS_UNKNOWN);
        }
        ...
        if (lock.tryLock()) {
            ...
            throw new QEException(DriverExceptionEnum.DRIVER_JOB_INTO_STORAGE_ERROR);
        }
    ...
    public void outStorage(DriverPickUpDTO driverPickUpDTO) {
        ...
        if (count > 0) {
            throw new QEException(DriverExceptionEnum.PROCESSING_DRIVER_JOB_NOT_EMPTY);
        }
        ...
        if (lock.tryLock()) {
            ...
            throw new QEException(DriverExceptionEnum.DRIVER_JOB_OUT_STORAGE_ERROR);
        }
    ...
    public PageResponse<DriverJobDTO> pageQuery(DriverJobPageQueryDTO dto) {
        ...
        //4.实体类转dto
        List<DriverJobDTO> list = pageResult.getRecords().stream().map(entity -> BeanUtil.toBean(entity, DriverJobDTO.class)).collect(Collectors.toList());
