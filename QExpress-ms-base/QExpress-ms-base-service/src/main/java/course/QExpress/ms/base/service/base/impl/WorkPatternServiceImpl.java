...
import course.QExpress.ms.base.mapper.base.WorkPatternMapper;
...
import course.QExpress.ms.base.utils.WorkPatternUtils;
...
public class WorkPatternServiceImpl extends ServiceImpl<WorkPatternMapper, WorkPatternEntity> implements WorkPatternService {
    ...
    public void delete(long id) {
        ...
        if (count > 0) {
            throw new QEException("改工作模式下有排班，请先把排班修改为其他工作模式后删除");
        }
        int number = getBaseMapper().deleteById(id);
        if (number <= 0) {
            throw new QEException("工作模式删除失败");
        }
    ...
    public void update(WorkPatternUpdateDTO workPatternUpdateDTO) {
        ...
        if (result <= 0) {
            throw new QEException("工作模式更新失败");
        }
    }
    ...
    public void add(WorkPatternAddDTO workPatternAddDTO) {
        ...
        if (result <= 0) {
            throw new QEException("工作模式新增失败");
        }
    }
